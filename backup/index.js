...
// ADD TASK (POST)
app.post('/add-task/submit', ensureAuthenticated, async (req, res) => {

    const { title, description, todo } = req.body;
    const userId = req.session.user.id;

    const sql = 'INSERT INTO tasks (user_id, title, description, todos) VALUES (?, ?, ?, ?)';

    try {
        await connection.promise().query(sql, [userId, title, description, JSON.stringify(todo)]);
        res.send(`<script>alert('Task berhasil ditambahkan!'); window.location.href = '/dashboard';</script>`);
    } catch (error) {
        console.error(error);
        res.status(500).send(`<script>alert('Gagal menambahkan task'); window.location.href = '/add-task';</script>`);
    }
});
